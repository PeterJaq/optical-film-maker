import optical_film.RunModeling
...
class optical_film_env(object):
    ...
    def init_Device(self, layer_num=3, mat_structure="LHL"):
        ...
        absorption = optical_film.RunModeling.cal_film_abs(Device=self.Device, libname=self.libname,
    ...
    def run_simulate(self, action):
        ...
        per_abs = optical_film.RunModeling.cal_film_abs(Device=self.Device, libname=self.libname,
                                                         WLstep=self.WLstep, posstep=self.posstep,
                                                         plotWL=self.plotWL, wavelength_range=self.wavelength_range)

        action_layer, action_num = self.action_op(action)

        mean_abs = optical_film.RunModeling.cal_film_abs(Device=self.Device, libname=self.libname,
                                                         WLstep=self.WLstep, posstep=self.posstep,
                                                         plotWL=self.plotWL, wavelength_range=self.wavelength_range)
        ...
        if mean_abs < self.lowest_abs:
            ...
            reward = -1
            #print(self.device_2_observation(self.Device))
        else:
            reward = 0
